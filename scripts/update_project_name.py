...
def get_files_to_update() -> list[Path]:
    ...
    optional_files = [
        "docs/backend-project-guide.md",
        "docs/ml-project-guide.md",
        "uv.lock",
    ]
...
def main() -> None:
    ...
    # Rename directory
    if args.dry_run:
        old_dir = Path(args.old_name)
        if old_dir.exists():
            print(f"Would rename: {old_dir} → {args.new_name}")
    ...
    if args.dry_run:
        ...
        print("  3. Review and update agents.md if needed")
